...
class Query:
    @strawberry.field
    async def books(self, limit: Optional[int] = None) -> List[Book]:
        async with models.get_session() as s:
            sql = select(models.Book).limit(limit).order_by(models.Book.name)
            db_book = (await s.execute(sql)).scalars().unique().all()
        return [Book.marshal(book) for book in db_book]

    @strawberry.field
    async def authors(self, limit: Optional[int] = None) -> List[Author]:
        async with models.get_session() as s:
            sql = select(models.Author).limit(limit).order_by(models.Author.name)
